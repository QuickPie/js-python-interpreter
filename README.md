...
> 这个项目教会我了好多好多，也让我对编程的理解不再浅浮于表面的的、“能运行”的代码上。我想对这个项目说声谢谢。
...
* 处理 Python 缩进和代码块时，我需要考虑缩进的丰富变化和各种代码块的模式，源码形式曾多次发生巨大改变。
* 处理 Python 异常时，我需要思考添加哪些属性和其兼容性，如何根据不同情况显示错误信息。
* 为了理解 Python 类型的底层机制，我反复参考 Python 的`builtins.py`文件和`builtins.pyi`文件，那些类型之间复杂的自指、继承、实例化等，曾阻挠了我三天三夜。

这些重重阻碍，不仅能提升我的 JavaScript 能力，**还让我对 Python 特性和语法有了更广、更深的挖掘和理解**。

**在一方面总结，这个项目对我而言，是一条掌握 JavaScript 最高效的路径——通过实现一个我已能熟练运用的复杂系统（Python 解释器）。**
...
**JavaScript-Python 解释器** - 一个用纯 JavaScript 从零实现的 Python 3.8 解释器，附有网页界面，在浏览器中直接运行。他可以是一个学习工具，更是一个深入理解编程语言底层逻辑（词法分析、语法分析、解释执行）的**绝佳示范**。对于使用这个项目有很高的自由度，你可以参阅代码、学习编译原理；如果你是个 Python 用户，相信你会对 Python 的底层机制有更深的了解；你也可以帮助我尝试为其添加更多 Python 语法（如循环、函数等）……
...
进入 Pages 页面（在线演示页面）时可能会显示“建立安全连接失败”，目前尚未解决。
